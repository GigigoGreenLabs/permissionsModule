...
public class LinearLayoutCompat extends ViewGroup {
  ...
  @IntDef(flag = true,
      value = {
          SHOW_DIVIDER_NONE, SHOW_DIVIDER_BEGINNING, SHOW_DIVIDER_MIDDLE, SHOW_DIVIDER_END
      }) @Retention(RetentionPolicy.SOURCE) public @interface DividerMode {
  }
  ...
  public LinearLayoutCompat(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    final TintTypedArray a =
        TintTypedArray.obtainStyledAttributes(context, attrs, R.styleable.LinearLayoutCompat,
            defStyleAttr, 0);
    ...
    boolean baselineAligned =
        a.getBoolean(R.styleable.LinearLayoutCompat_android_baselineAligned, true);
    if (!baselineAligned) {
   ...
   */
  @DividerMode public int getShowDividers() {
      ...
      if (majorGravity != Gravity.TOP) {
        switch (majorGravity) {
          ...
          case Gravity.CENTER_VERTICAL:
            childTop +=
                ((getBottom() - getTop() - getPaddingTop() - getPaddingBottom()) - mTotalLength)
                    / 2;
            break;
        }
      }
  ...
  void measureVertical(int widthMeasureSpec, int heightMeasureSpec) {
    ...
    for (int i = 0; i < count; ++i) {
      ...
      if (lp.weight > 0) {
        ...
        alternativeMaxWidth =
            Math.max(alternativeMaxWidth, matchWidthLocally ? margin : measuredWidth);
      }
    ...
    if (skippedMeasure || delta != 0 && totalWeight > 0.0f) {
      ...
      for (int i = 0; i < count; ++i) {
        ...
        boolean matchWidthLocally =
            widthMode != MeasureSpec.EXACTLY && lp.width == LayoutParams.MATCH_PARENT;
    ...
    for (int i = 0; i < count; ++i) {
      ...
      if (baselineAligned) {
        ...
        if (childBaseline != -1) {
          ...
          final int gravity =
              (lp.gravity < 0 ? mGravity : lp.gravity) & Gravity.VERTICAL_GRAVITY_MASK;
      ...
      if (lp.weight > 0) {
        ...
        alternativeMaxHeight =
            Math.max(alternativeMaxHeight, matchHeightLocally ? margin : childHeight);
      }
    ...
    if (skippedMeasure || delta != 0 && totalWeight > 0.0f) {
      ...
      for (int i = 0; i < count; ++i) {
        ...
        boolean matchHeightLocally =
            heightMode != MeasureSpec.EXACTLY && lp.height == LayoutParams.MATCH_PARENT;
   ...
   */
  @OrientationMode public int getOrientation() {
    return mOrientation;
  }
